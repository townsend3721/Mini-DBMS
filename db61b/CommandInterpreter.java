...
class CommandInterpreter {
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            ...
            ArrayList<String> list_columnName = new ArrayList<>();
            list_columnName.add(_columnName);
            while (_input.nextIf(",")) {
                list_columnName.add(columnName());
                }
            String[] newColumnName =  new String[list_columnName.size()];
            for (int i = 0; i < list_columnName.size(); i++) {
                newColumnName[i] = list_columnName.get(i);
            }
            _input.next(")");
            table = new Table(newColumnName);
            return table;
        }
        else {
            _input.next("as");
    ...
    Table selectClause() {
        ...
        ArrayList<Condition> list_condition;
        Table _t_3;
        list_titles.add(columnName());
        while (_input.nextIf(",")) {
            list_titles.add(columnName());
        }
        _input.next("from");
        ...
        if (_input.nextIf(",")) {
            Table _t2 = tableName();
            list_condition = conditionClause(_t1, _t2);
            _t_3 = _t1.select(_t2, list_titles, list_condition);
        }
        else {
            list_condition = conditionClause(_t1);
            _t_3 = _t1.select(list_titles, list_condition);
        }
        return _t_3;
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ArrayList<Condition> _conditionList = new ArrayList<>();
        if (!_input.nextIf("where")) {
            return _conditionList;}
        _conditionList.add(condition(tables));
